...
local function TestAllSame()
	print("\ntesting if all rolls are the same")
	local a = meowth.Match:New()
	a:AddGambler("Squirtle")
	a:AddGambler("Charmander")
	a:AddGambler("Bulbasaur")
	a:AddGambler("Gengar")
	a:Start()
	a:AddRoll("Squirtle", 7)
	a:AddRoll("Charmander", 7)
	a:AddRoll("Bulbasaur", 7)
	a:AddRoll("Gengar", 7)
	assert(a.phase == "all match", "phase should be all match")
	a:Reroll()
	assert(a.phase == "roll", "phase should be roll now")
	assert(CompareArrays({"Squirtle", "Charmander", "Bulbasaur", "Gengar"}, a:GetWaitingList()),
		"the waiting list should be everyone again")
end
